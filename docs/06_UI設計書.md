...
<svg
	class={cn('text-primary-500 animate-spin', sizes[size], className)}
...
{#if $page.error}
	<div class="flex min-h-[400px] flex-col items-center justify-center gap-4">
		<h1 class="text-2xl font-bold text-neutral-900">エラーが発生しました</h1>
...
<div class="flex flex-col items-center justify-center py-12 text-center">
	{#if Icon}
		<Icon class="mb-4 h-12 w-12 text-neutral-400" />
	{/if}
	<h3 class="text-lg font-medium text-neutral-900">{title}</h3>
	{#if description}
		<p class="mt-1 text-neutral-500">{description}</p>
	{/if}
	{#if actionLabel}
